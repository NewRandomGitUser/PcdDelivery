...
public class EntryPoint {
    ...
    public static void ExecuteDemonstration(){
        GameOfLife game = new GameOfLife(2048, 1);
        game.printDemonstration();
    }

    public static TimeVariation ExecuteGameOfLifeNThreads(int nThreads) {
        TimeVariation timeVariation = new TimeVariation();
        long start = System.currentTimeMillis();
        GameOfLife game = new GameOfLife(2048, nThreads);
    ...
    public static void main(String [] args) {
        ...
        for(var number: numberOfThreadsPerExecution) {
            timeVariationPerExecution.add(ExecuteGameOfLifeNThreads(number));
        }
        for(int i = 0; i<4; i++) {
